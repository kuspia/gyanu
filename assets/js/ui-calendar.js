...
export function createCalendarView({ store }) {
  ...
  const detailNode = el('div', { class: 'day-detail' });
  ...
  async function selectDay(dateKey) {
    if (!isViewable(dateKey)) return;
    selected = dateKey;
    for (const node of gridNode.querySelectorAll('.day')) {
      node.classList.toggle('is-selected', node.dataset.date === dateKey);
    }
    // A day that is not in the logged list is known to be empty. Fetching it
    // would only ever 404, so the network is skipped entirely.
    let doc = loggedDates.includes(dateKey) ? entryCache.get(dateKey) : null;
    if (doc === undefined) {
      mount(detailNode, [el('p', { class: 'detail-loading', text: 'Loading…' })]);
      try {
        doc = await store.readEntryForDisplay(dateKey);
      } catch {
        doc = null;
      }
      entryCache.set(dateKey, doc);
    }
    if (selected !== dateKey) return;

    if (!doc) {
      mount(detailNode, [
        el('div', { class: 'detail-card detail-card--missed' }, [
          el('div', { class: 'detail-head' }, [
            el('h3', { text: formatDateKey(dateKey) }),
            el('span', { class: 'chip chip--bad', text: 'Cheat day' })
          ]),
          el('p', { text: 'Nothing was logged for this day, and the window for it closed at midnight. It stays a blank on the record permanently — it cannot be filled in later.' })
        ])
      ]);
      return;
    }
    ...
    mount(detailNode, [
      el('div', { class: 'detail-card' }, [
        el('div', { class: 'detail-head' }, [
          el('h3', { text: formatDateKey(dateKey) }),
          el('span', { class: 'chip', text: `Up at ${formatWakeTime(doc.wakeUpTime)}` })
        ]),
        entryDetail(doc),
        el('p', { class: 'entry-footnote', text: `Submitted ${doc.submittedAt ?? 'unknown'}` })
      ])
    ]);
  }
  ...
  async function refresh({ focus } = {}) {
    ...
    renderStats();
    renderMonth();
    if (focus && isViewable(focus)) selectDay(focus);
  }
